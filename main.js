...
// Robust markdown renderer using marked.js (loaded from CDN in index.html)
// Falls back to a simple inline renderer if marked is not available
function renderMarkdown(markdown) {
    if (!markdown) return '';

    // Use marked.js if available (preferred)
    ...
    // Fallback: simple but reliable renderer
    let html = String(markdown);

    // Protect fenced code blocks
    const codeBlocks = [];
    html = html.replace(/```(\w*)\n?([\s\S]*?)```/g, (_, lang, code) => {
        const placeholder = `\x02CODE${codeBlocks.length}\x03`;
    ...
    html = html.replace(/`([^`]+)`/g, (_, code) => {
        const placeholder = `\x02INLINE${inlineCodes.length}\x03`;
    ...
    // Headings (h4 → h1, in reverse order to avoid conflicts)
    ...
    // Convert lines to structured blocks, then handle lists and paragraphs
    ...
    while (i < lines.length) {
        ...
        // Already-tagged HTML or placeholder — pass through
        if (line.startsWith('<') || /^\x02/.test(line)) {
            result.push(line);
            i++;
            continue;
        }
    ...
    codeBlocks.forEach((block, idx) => {
        html = html.replace(`\x02CODE${idx}\x03`, block);
    });
    inlineCodes.forEach((code, idx) => {
        html = html.replace(`\x02INLINE${idx}\x03`, code);
    });
